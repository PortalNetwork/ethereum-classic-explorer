...
class App extends Component {
    ...
    handSeachData=()=>{
        ...
        const seachdamain = domain[domain.length-1];     //去頭去尾去.etc
        if(seachdamain.length < 6) return this.handOpenWarning("ECNS has the minimum character length of 6");
        ...
        getEntries(seachdamain).then(entries => {
            getOwner(entries.deed).then(owner => {
                ...
                this.setState({
                    entries: eObj,
                    idxRes: t
                },()=>this.overResolver(`${seachdamain}.etc`))
            });
        });
    ...
    render() {
        return (
            <div className="ethereum-classic">
                <h1>ECNS Explorer</h1>
                <div className="seach">
                    <input type="text" 
                ...
                <div className="urllink">
                    ...
                    <a href="https://github.com/PortalNetwork/ethereum-classic-explorer" target="_blank"><i className="fab fa-github fa-3x"></i></a>
                </div>

                <a className="textabout" onClick={this.handOpenAboutChange}>Find out more about ECNS</a>

                {this.state.isAboutOpen && 
                    <div className="info">
                        <a onClick={this.handOpenAboutChange} className="closeInfo"><i className="fas fa-times-circle"></i></a>
                        <p>ECNS is the Ethereum Classic Name Service which is a distributed, extensible naming system based on the Ethereum Classic blockchain that can be used to resolve a wide variety of resources such as Ethereum Classic addresses.</p>
                    </div>
                }
